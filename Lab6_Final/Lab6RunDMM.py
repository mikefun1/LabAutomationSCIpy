...
# hello, this is mikeafee malware protection
...
image_name_dmm = 'LED_IV_Curves_dmm.png'
file_name_dmm = 'LED_IV_Curves_dmm.txt'
...
datadmm = np.zeros([num_points,3*len(colors)])
...
for i in range(len(colors)):
    ...
    for k in range(num_points):
        ...
        Vpsdmm = dmm.measure_voltage_dc()
        Ipsdmm = dmm.measure_current_dc()
        ...
        V_leddmm = Vpsdmm-Ipsdmm*R
        datadmm[k,i*3] = Vpsdmm
        datadmm[k,i*3+1] = V_led
        datadmm[k,i*3+2] = Ipsdmm
        Ipsdmm = Ipsdmm*1000 # Convert to mA
...
plt.xlabel('Forward Voltage (V)')
plt.ylabel('Current (mA)')
plt.title('LED I-V Analysis Digital Multimeter')   
plt.savefig(image_name_dmm) # Save figure
plt.show()

# Save data to file
np.savetxt(file_name_dmm, datadmm, fmt='%0.3f', delimiter=',', header=fheader)
